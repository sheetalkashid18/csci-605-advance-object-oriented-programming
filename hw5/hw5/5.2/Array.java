...
class Array {
	...
	// Holds the elements added so far in an Array object
	int soMany = 0;
	
	// If the Array has flexible size
	boolean isFlexible = false;
	
	// SimpleDateFormat object to store and print the 
	// creation and modification time of an Array object
	static final SimpleDateFormat df = new SimpleDateFormat("yyyy-MMM-dd hh:mm:ss aa");
	
	// Assumed that only a single null object can be added 
	// and contains if it has already been added
	boolean isNullObjectAdded = false;
	...
	// Deletes the object specified from the Array object
